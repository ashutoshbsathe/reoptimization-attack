...
from mnist_net import mnist_net as SmallCNN
...
model = SmallCNN().cuda()
model.load_state_dict(torch.load('./models/pgd_madry.pth'))
model.eval()

sub = SmallCNN().cuda()
sub.load_state_dict(torch.load('./substitute_models/mnist_pgd_madry.pt'))
sub.eval()
